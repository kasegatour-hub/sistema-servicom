...
export function formatShipmentAmount(amount: number | string | null | undefined, shipment: Parameters<typeof resolveShipmentPricingCurrency>[0]) {
  const numeric = Number(amount);
  if (!Number.isFinite(numeric)) return "No especificado";
  return `${numeric.toFixed(2)} ${resolveShipmentPricingCurrency(shipment) === "PEN" ? "S/" : "EUR"}`;
}
