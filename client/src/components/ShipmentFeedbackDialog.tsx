...
type ShipmentFeedbackDialogProps = {
  shipment: { id: number; orderNumber: string; code: string } | null;
  open: boolean;
...
export function ShipmentFeedbackDialog({ shipment, open, onOpenChange }: ShipmentFeedbackDialogProps) {
  const [message, setMessage] = useState("");
  const [file, setFile] = useState<File | null>(null);
  const feedback = trpc.feedback.list.useQuery({ shipmentId: shipment?.id || 0 }, { enabled: open && Boolean(shipment) });
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!shipment || !message.trim()) return;
    ...
    createFeedback.mutate({ shipmentId: shipment.id, message: message.trim(), attachment });
  };

  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
      <DialogHeader>
        <DialogTitle className="flex items-center gap-2 text-[#0B2B5E]"><MessageSquare className="h-5 w-5" /> Retroalimentación del envío</DialogTitle>
        {shipment && <DialogDescription>Orden {shipment.orderNumber} · Código {shipment.code}. Comparte un comentario y, si corresponde, una evidencia multimedia.</DialogDescription>}
      </DialogHeader>
      <div className="space-y-3" aria-label="Historial de retroalimentación">
        {feedback.isLoading ? <p className="text-sm text-slate-500">Cargando mensajes…</p> : feedback.data?.length ? feedback.data.map((item: any) => <article key={item.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><strong className="text-sm text-[#0B2B5E]">{item.authorLabel}</strong><time className="text-xs text-slate-500">{new Date(item.createdAt).toLocaleString()}</time></div><p className="mt-2 whitespace-pre-wrap text-sm text-slate-700">{item.message}</p>{previewAttachment(item)}</article>) : <p className="rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-500">Aún no hay retroalimentación para este envío.</p>}
      </div>
      <form onSubmit={submit} className="space-y-3 border-t pt-4">
        <label className="block text-sm font-semibold text-slate-700">Escribe tu retroalimentación</label>
        <Textarea value={message} onChange={event => setMessage(event.target.value)} maxLength={2000} required placeholder="Describe una consulta, comentario o incidencia del envío…" rows={4} />
        ...
        <div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Cerrar</Button><Button type="submit" disabled={createFeedback.isPending} className="bg-[#0B2B5E] text-white">{createFeedback.isPending ? "Enviando…" : <><Send className="mr-2 h-4 w-4" /> Enviar retroalimentación</>}</Button></div>
      </form>
    </DialogContent>
  </Dialog>;
}
