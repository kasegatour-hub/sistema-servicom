...
export function DocumentPricePreview({ docType, sheetCount, additionalTotalEur = 0, manualPriceEur, extraPriceEur = 0 }: DocumentPricePreviewProps) {
  const price = getDocumentPricePreview({ docType, sheetCount, additionalTotalEur, manualPriceEur, extraPriceEur });
    ...
    <div key={`${price.docType}-${price.sheetCount}-${price.totalEur}-${price.usesManualPrice}`} aria-live="polite" className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-950 shadow-sm transition-all duration-200">
      ...
      {price.usesManualPrice ? (
        ...
        <div className="mt-3 space-y-1 text-sm">
          ...
          {price.extraPriceEur > 0 && <p className="font-semibold text-emerald-800">Importe extra: <strong>+{price.extraPriceEur.toFixed(2)} €</strong>.</p>}
        </div>
      )}
      {price.usesManualPrice && price.extraPriceEur > 0 && <p className="mt-2 text-sm font-semibold text-emerald-800">Importe extra: +{price.extraPriceEur.toFixed(2)} €.</p>}
    </div>
  );
}
