...
export function DocumentPricePreview({ docType, sheetCount, additionalTotalEur = 0, manualPriceEur }: DocumentPricePreviewProps) {
  const price = getDocumentPricePreview({ docType, sheetCount, additionalTotalEur, manualPriceEur });
  ...
  return (
    <div key={`${price.docType}-${price.sheetCount}-${price.totalEur}-${price.usesManualPrice}`} aria-live="polite" className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-950 shadow-sm transition-all duration-200">
      ...
      {price.usesManualPrice ? (
        <p className="mt-3 text-sm font-medium text-amber-900">Precio manual activo: reemplaza la tarifa automática y los documentos adicionales.</p>
      ) : (
