...
export function AgencyDestinationPicker({ route, value, onChange, provider, onProviderChange, hideServicomDestination = false }: { route: string; value: string; onChange: (value: string) => void; provider?: AgencyProvider; onProviderChange?: (provider: AgencyProvider) => void; hideServicomDestination?: boolean }) {
  ...
  const isProvinceRoute = hideServicomDestination || isProvinceShipmentRoute(route);
  useEffect(() => { if (!isProvinceRoute && !value.trim()) onChange(defaultDestination(route)); }, [route, value, onChange, isProvinceRoute]);
  ...
  return <section className="rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-50 via-white to-slate-50 p-4 shadow-sm md:col-span-3"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h4 className="flex items-center gap-2 font-semibold text-[#0B2B5E]"><Building2 className="h-4 w-4" />Agencia de destino</h4><p className="mt-1 max-w-3xl text-xs text-slate-600">{isProvinceRoute ? <>Elige la agencia provincial de Olva, Shalom, FedEx, DHL u otra empresa regional para completar el destino.</> : <>Primero conserva la sede regular o pulsa <strong>Elegir agencia</strong>. Recién entonces se mostrarán Olva, Shalom, FedEx, DHL y las empresas regionales.</>}</p></div><Button type="button" size="sm" variant="outline" onClick={() => setShowExplorer(open => !open)} className="border-[#0B2B5E] bg-white text-[#0B2B5E]"><MapPin className="mr-1 h-4 w-4" />{showExplorer ? "Cerrar directorio" : "Elegir agencia"}</Button></div><div className="mt-3 flex flex-col gap-2 sm:flex-row">{!isProvinceRoute && <Button type="button" size="sm" variant="outline" onClick={selectServicom} className="border-blue-300 bg-white text-[#0B2B5E]">Usar sede Servicom</Button>}<output aria-live="polite" className="min-h-9 flex-1 rounded-md border border-blue-200 bg-white px-3 py-2 text-sm font-medium text-slate-700">{value || (isProvinceRoute ? "Selecciona una agencia provincial" : defaultDestination(route))}</output></div>{showExplorer && <AgencyDirectoryExplorer providerValue={selectedProvider} onProviderChange={selectProvider} onSelect={onChange} onMessage={setMessage} onClose={() => setShowExplorer(false)} />}{message && <p role="status" className="mt-2 rounded-lg bg-blue-50 px-3 py-2 text-sm font-medium text-[#0B2B5E]">{message}</p>}</section>;
}
