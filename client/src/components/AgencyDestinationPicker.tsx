...
type ProviderFilter = "olva" | "shalom" | "fedex" | "dhl";
type Agency = { id: string; provider: "OLVA COURIER" | "SHALOM" | "FEDEX" | "DHL"; name: string; address: string; department: string; province: string; district: string; kind: string; latitude: number | null; longitude: number | null; phone?: string; businessHours?: string; sundayHours?: string; sourceUrl: string };
...
function AgencyDirectoryExplorer({ onSelect, onMessage, onClose }: { onSelect: (value: string) => void; onMessage: (value: string) => void; onClose: () => void }) {
  ...
  useEffect(() => {
    if (!isGlobalCarrier || !mapReady || !mapRef.current || query.trim().length < 2) {
      setCarrierAgencies([]);
    ...
    return () => { active = false; };
  }, [isGlobalCarrier, mapReady, provider, providerName, query]);
  const agencies = (isGlobalCarrier ? carrierAgencies : currentQuery.data?.agencies || []) as Agency[];
  const directoryLoading = isGlobalCarrier ? carrierLoading : currentQuery.isLoading;
  const directoryError = isGlobalCarrier ? carrierError : currentQuery.isError;

  useEffect(() => { setVisibleCount(25); setSelectedAgency(null); }, [provider, query]);
  useEffect(() => {
    if (!mapRef.current) return;
    directoryMarkersRef.current.forEach(marker => { marker.map = null; });
    directoryMarkersRef.current = agencies.slice(0, 180).filter(agency => agency.latitude !== null && agency.longitude !== null).map(agency => new google.maps.marker.AdvancedMarkerElement({ map: mapRef.current!, position: { lat: agency.latitude!, lng: agency.longitude! }, title: `${agency.name} — ${agency.address}` }));
  }, [agencies]);
