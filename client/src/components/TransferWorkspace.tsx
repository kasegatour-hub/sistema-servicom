...
import { getRoutePresentation, ROUTES } from "@/lib/routeDetails";
...
type TransferRoute = typeof ROUTES.LIMA_TORINO | typeof ROUTES.TORINO_LIMA;
...
const emptyForm: TransferForm = {
  route: "", senderName: "", senderPhone: "", senderDocument: "", senderDocumentType: "dni_peru", senderPaymentMethod: "Agencia",
...
export function getTransferRouteOffices(route: TransferRoute, branding = getReceiptBranding({})) {
...
export function TransferWorkspace({ admin, initialForm, mobileRegistrationMode = false }: { admin?: { id?: number; email?: string } | null; initialForm?: Partial<TransferForm>; mobileRegistrationMode?: boolean }) {
  const [form, setForm] = React.useState<TransferForm>(() => ({ ...emptyForm, ...initialForm }));
  const [errors, setErrors] = React.useState<TransferErrors>({});
  const [createdTransfer, setCreatedTransfer] = React.useState<any>(null);
  const [deliveryReceiptTransfer, setDeliveryReceiptTransfer] = React.useState<any>(null);
  ...
  const usesArgenper = form.currency !== form.destinationCurrency;
  const argenperQuote = trpc.transfers.argenperQuote.useQuery(undefined, { retry: false, enabled: usesArgenper });
  const hasArgenperQuote = Boolean(argenperQuote.data?.adjustedPenPerEur);
  ...
  React.useEffect(() => {
    if (!usesArgenper || hasArgenperQuote || !argenperQuote.error || form.exchangeRateSource !== "argenper") return;
    setForm(current => ({ ...current, exchangeRateSource: "manual" }));
  }, [usesArgenper, hasArgenperQuote, argenperQuote.error, form.exchangeRateSource]);
  ...
  const setSourceCurrency = (currency: TransferCurrency) => setForm(current => {
    const destinationCurrency = currency === "PEN" ? "EUR" : current.destinationCurrency;
    const needsConversion = currency !== destinationCurrency;
    return { ...current, currency, destinationCurrency, exchangeRate: needsConversion ? (argenperQuote.data?.adjustedPenPerEur ? String(argenperQuote.data.adjustedPenPerEur) : current.exchangeRate) : "1", exchangeRateSource: needsConversion ? "argenper" : "paridad" };
  });
  const setDestinationCurrency = (destinationCurrency: TransferCurrency) => setForm(current => {
    const needsConversion = current.currency !== destinationCurrency;
    return { ...current, destinationCurrency, exchangeRate: needsConversion ? (argenperQuote.data?.adjustedPenPerEur ? String(argenperQuote.data.adjustedPenPerEur) : current.exchangeRate) : "1", exchangeRateSource: needsConversion ? "argenper" : "paridad" };
  });
  ...
  const submit = (event: React.FormEvent) => {
    ...
    create.mutate({ ...form, route: form.route as TransferRoute, amountSent: Number(form.amountSent), transferFee, exchangeRate: Number(form.exchangeRate), recipientBank: form.recipientBank || undefined, recipientIban: form.recipientIban || undefined, recipientCci: form.recipientCci || undefined });
  };
  ...
  return <div className="space-y-6">
    <Card className="border-2 border-emerald-300 bg-emerald-50/40 p-5 shadow-lg"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-800">Operaciones</p><h2 className="text-2xl font-extrabold text-emerald-950">Transferencia</h2><p className="mt-1 max-w-2xl text-sm text-slate-600">Registra la identidad y el teléfono completo de ambas personas. Banco, IBAN y CCI son opcionales.</p></div><Receipt className="h-8 w-8 text-emerald-700" /></div>
      <form onSubmit={submit} className="mt-5 space-y-5" noValidate>
        {mobileRegistrationMode && <div className="rounded-xl border-2 border-emerald-300 bg-white p-4" role="region" aria-label="Pasos de transferencia móvil"><div className="flex items-center justify-between gap-2"><p className="text-sm font-extrabold text-emerald-900">Registro móvil de transferencia</p><span className="text-xs font-bold text-slate-500">Paso {mobileStep} de 3</span></div><div className="mt-3 grid grid-cols-3 gap-2">{([1, 2, 3] as const).map(step => <button key={step} type="button" onClick={() => setMobileStep(step)} className={`min-h-11 rounded-lg px-2 text-xs font-bold transition ${mobileStep === step ? "bg-emerald-700 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`} aria-current={mobileStep === step ? "step" : undefined}>{step === 1 ? "Ruta" : step === 2 ? "Personas" : "Importe"}</button>)}</div><div className="mt-3 h-2 overflow-hidden rounded-full bg-emerald-100"><div className="h-full rounded-full bg-emerald-600 transition-all" style={{ width: `${mobileStep * 33.333}%` }} /></div><p className="mt-2 text-xs leading-5 text-slate-600">{mobileStep === 1 ? "Elige la ruta y verifica las sedes." : mobileStep === 2 ? "Completa remitente y destinatario." : "Indica importe, comisión, estado y notas."}</p></div>}
        {(!mobileRegistrationMode || mobileStep === 1) && <div className="rounded-xl border border-emerald-200 bg-emerald-50/70 p-4"><label className="block text-sm font-semibold text-slate-700">Ruta de transferencia *<Select value={form.route} onValueChange={value => { const route = value as TransferRoute; setForm(current => ({ ...current, route })); setErrors(current => ({ ...current, route: undefined })); }}><SelectTrigger className={`mt-1 h-12 bg-white ${errors.route ? "border-rose-500 ring-1 ring-rose-200" : ""}`} aria-invalid={Boolean(errors.route)}><SelectValue placeholder="Elige Lima–Torino o Torino–Lima" /></SelectTrigger><SelectContent><SelectItem value={ROUTES.LIMA_TORINO}>Lima – Torino</SelectItem><SelectItem value={ROUTES.TORINO_LIMA}>Torino – Lima</SelectItem></SelectContent></Select></label>{errors.route && <p role="alert" className="mt-1 text-xs font-semibold text-rose-700">{errors.route}</p>}{routeOffices ? <div className="mt-3 grid gap-3 text-sm md:grid-cols-2"><div className="rounded-lg border border-emerald-100 bg-white p-3"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Sede de origen</p><p className="mt-1 font-extrabold text-emerald-900">{routeOffices.origin.label}</p><p className="mt-1 text-xs leading-5 text-slate-600">{routeOffices.origin.address}<br />{routeOffices.origin.phone}</p></div><div className="rounded-lg border border-emerald-100 bg-white p-3"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Sede de destino</p><p className="mt-1 font-extrabold text-emerald-900">{routeOffices.destination.label}</p><p className="mt-1 text-xs leading-5 text-slate-600">{routeOffices.destination.address}<br />{routeOffices.destination.phone}</p></div></div> : <p className="mt-2 text-xs text-slate-600">Selecciona la ruta para ver la sede de origen y la sede de destino correctas.</p>}</div>}
        {(!mobileRegistrationMode || mobileStep === 2) && <><div className="rounded-xl border border-emerald-200 bg-emerald-50/70 p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="text-lg font-bold text-emerald-900">Remitente</h3><p className="text-xs text-slate-600">Datos del cliente obligatorios.</p></div><span className="rounded-full bg-white px-3 py-1 text-xs font-bold text-emerald-800">1 de 2</span></div><div className="mt-3 grid gap-3 md:grid-cols-3">{field("senderName", "Nombre completo", "Ej. Gisella Rocío Velásquez Coronado", true)}<IdentityDocumentField id="transfer-sender-document" label="Documento de identidad del remitente" documentType={form.senderDocumentType} onDocumentTypeChange={value => set("senderDocumentType", value)} value={form.senderDocument} onValueChange={value => set("senderDocument", value)} required error={documentError("senderDocument")} /><div><label className="block text-sm font-semibold text-slate-700">Teléfono / WhatsApp *</label><PhoneInput id="transfer-sender-phone" value={form.senderPhone} onChange={value => set("senderPhone", value)} placeholder="970 188 447" required className="mt-1" />{errors.senderPhone && <p role="alert" className="mt-1 text-xs font-semibold text-rose-700">{errors.senderPhone}</p>}</div><label className="block text-sm font-semibold text-slate-700">Modalidad de pago<Select value={form.senderPaymentMethod} onValueChange={value => set("senderPaymentMethod", value)}><SelectTrigger aria-label="Modalidad de pago" className="mt-1 h-11 bg-white"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Agencia">Agencia</SelectItem><SelectItem value="Banca">Banca</SelectItem></SelectContent></Select></label></div></div><div className="rounded-xl border border-emerald-200 bg-emerald-50/70 p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="text-lg font-bold text-emerald-900">Destinatario</h3><p className="text-xs text-slate-600">Datos del cliente obligatorios.</p></div><span className="rounded-full bg-white px-3 py-1 text-xs font-bold text-emerald-800">2 de 2</span></div><div className="mt-3 grid gap-3 md:grid-cols-3">{field("recipientName", "Nombre completo", "Ej. Maruja Coronado Cárdenas", true)}<IdentityDocumentField id="transfer-recipient-document" label="Documento de identidad del destinatario" documentType={form.recipientDocumentType} onDocumentTypeChange={value => set("recipientDocumentType", value)} value={form.recipientDocument} onValueChange={value => set("recipientDocument", value)} required error={documentError("recipientDocument")} /><div><label className="block text-sm font-semibold text-slate-700">Teléfono / WhatsApp *</label><PhoneInput id="transfer-recipient-phone" value={form.recipientPhone} onChange={value => set("recipientPhone", value)} placeholder="970 188 447" required className="mt-1" />{errors.recipientPhone && <p role="alert" className="mt-1 text-xs font-semibold text-rose-700">{errors.recipientPhone}</p>}</div></div><div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 p-3"><p className="text-sm font-bold text-emerald-900">Datos bancarios opcionales</p><p className="mt-1 text-xs leading-5 text-emerald-800">Banco, IBAN y CCI no bloquean el registro. Agrégalos si el cliente ya los proporcionó.</p><div className="mt-3 grid gap-3 md:grid-cols-3">{field("recipientBank", "Banco destinatario")}{field("recipientIban", "IBAN")}{field("recipientCci", "CCI")}</div></div></div></>}
        {(!mobileRegistrationMode || mobileStep === 3) && <div className="rounded-xl border border-emerald-200 bg-emerald-50/70 p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="text-lg font-bold text-emerald-900">Datos de transferencia</h3><p className="mt-1 text-xs text-slate-600">La comisión se calcula automáticamente según el par de monedas.</p></div><BadgePercent className="h-6 w-6 text-emerald-700" /></div><div className="mt-3 grid gap-3 md:grid-cols-5">{field("amountSent", `Importe enviado (${form.currency})`, "0,00", true)}<label className="block text-sm font-semibold text-slate-700">Moneda enviada<Select value={form.currency} onValueChange={value => setSourceCurrency(value as TransferCurrency)}><SelectTrigger aria-label="Moneda enviada" className="mt-1 h-11 bg-white"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="EUR">EUR — Euro</SelectItem><SelectItem value="PEN">PEN — Sol peruano</SelectItem></SelectContent></Select></label><label className="block text-sm font-semibold text-slate-700">Moneda que recibe<Select value={form.destinationCurrency} onValueChange={value => setDestinationCurrency(value as TransferCurrency)}><SelectTrigger aria-label="Moneda que recibe" className="mt-1 h-11 bg-white"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="EUR">EUR — Euro</SelectItem><SelectItem value="PEN">PEN — Sol peruano</SelectItem></SelectContent></Select></label><div className="rounded-lg bg-white p-3 ring-1 ring-emerald-200"><span className="block text-xs font-bold uppercase text-emerald-900">Comisión automática</span><strong className="text-xl text-emerald-950">{commissionPercent.toFixed(2)}%</strong><span className="mt-1 block text-xs text-emerald-900">{money(transferFee)} {form.currency}</span></div><div className="rounded-lg bg-cyan-100 p-3"><span className="block text-xs font-bold uppercase text-cyan-900">Importe recibido</span><strong className="text-xl text-cyan-950">{amountReceived.toFixed(2)} {form.destinationCurrency}</strong></div></div>{usesArgenper ? <div className="mt-3 rounded-xl border border-cyan-200 bg-cyan-50 p-3"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="flex items-center gap-2 text-sm font-bold text-cyan-950"><ArrowRightLeft className="h-4 w-4" />Cotización {form.currency} → {form.destinationCurrency}</p><p className="mt-1 text-xs text-cyan-800">Referencia Argemper: venta {hasArgenperQuote ? money(argenperQuote.data!.eurSaleRate) : "…"} + 0,15 = {hasArgenperQuote ? money(argenperQuote.data!.adjustedPenPerEur) : "…"} PEN por EUR.</p></div><Button type="button" variant="outline" onClick={() => { setForm(current => ({ ...current, exchangeRateSource: "argenper" })); void argenperQuote.refetch(); }} disabled={argenperQuote.isFetching} className="border-cyan-300 bg-white text-cyan-900"><RefreshCw className={`mr-2 h-4 w-4 ${argenperQuote.isFetching ? "animate-spin" : ""}`} />Actualizar Argemper</Button></div><div className="mt-3 grid gap-3 md:grid-cols-2"><label htmlFor="transfer-exchange-rate" className="text-sm font-semibold text-slate-700">Tipo de cambio aplicado (PEN por EUR)<Input id="transfer-exchange-rate" inputMode="decimal" value={form.exchangeRate} onChange={event => { set("exchangeRate", event.target.value.replace(/[^0-9.,]/g, "").replace(",", ".")); set("exchangeRateSource", "manual"); }} className="mt-1 h-11 bg-white" /></label><div className="rounded-lg bg-white p-3 text-sm text-cyan-950"><strong>{form.exchangeRateSource === "argenper" && hasArgenperQuote ? "Cotización Argemper aplicada" : "Tipo de cambio manual"}</strong><p className="mt-1 text-xs text-slate-600">Puedes escribir un valor distinto si acuerdas otro tipo de cambio con el cliente.</p></div></div>{!hasArgenperQuote && argenperQuote.error && <p role="alert" className="mt-2 text-xs font-semibold text-amber-800">No fue posible actualizar Argenper en este momento. Ingresa el tipo de cambio manual acordado y continúa.</p>}</div> : <div className="mt-3 rounded-xl border border-slate-200 bg-white p-3 text-sm text-slate-700"><strong>{form.currency} → {form.destinationCurrency}:</strong> se aplica comisión automática de 3 % y tipo de cambio 1,00.</div>}<div className="mt-3 grid gap-3 md:grid-cols-2"><label className="text-sm font-semibold text-slate-700">Estado<Select value={form.status} onValueChange={value => set("status", value)}><SelectTrigger className="mt-1 h-11 bg-white"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Registrada">Registrada</SelectItem><SelectItem value="Pagada">Pagada</SelectItem><SelectItem value="Cancelada">Cancelada</SelectItem></SelectContent></Select></label><label htmlFor="transfer-notes" className="text-sm font-semibold text-slate-700">Notas<Textarea id="transfer-notes" value={form.notes} onChange={event => set("notes", event.target.value)} className="mt-1 bg-white" placeholder="Observaciones" /></label></div></div>}
        ...
        <div className="flex flex-wrap justify-end gap-3">{mobileRegistrationMode && mobileStep > 1 && <Button type="button" variant="outline" onClick={() => setMobileStep(current => (current - 1) as 1 | 2 | 3)}>Atrás</Button>}{mobileRegistrationMode && mobileStep < 3 && <Button type="button" onClick={() => setMobileStep(current => (current + 1) as 1 | 2 | 3)} className="bg-emerald-700 text-white hover:bg-emerald-800">Siguiente</Button>}<Button type="button" variant="outline" onClick={() => { setForm(emptyForm); setErrors({}); setCreatedTransfer(null); setMobileStep(1); }}><RefreshCw className="mr-2 h-4 w-4" />Limpiar</Button><Button type="submit" disabled={create.isPending} className={`min-h-12 bg-emerald-700 px-6 text-white hover:bg-emerald-800 ${mobileRegistrationMode && mobileStep !== 3 ? "hidden" : ""}`}>{create.isPending ? "Guardando…" : "Crear transferencia"}</Button></div>
      </form>
    </Card>
    {createdTransfer && <Card className="border border-emerald-200 bg-emerald-50 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold text-emerald-800">Transferencia creada correctamente</p><p className="text-2xl font-extrabold text-emerald-950">{createdTransfer.transferNumber}</p></div><div className="flex flex-wrap gap-2"><Button type="button" onClick={() => printTransfer(createdTransfer)} className="bg-[#0B2B5E] text-white"><Printer className="mr-2 h-4 w-4" />Imprimir recibo</Button><Button type="button" variant="outline" onClick={() => setDeliveryReceiptTransfer(createdTransfer)} className="border-emerald-700 text-emerald-800"><Receipt className="mr-2 h-4 w-4" />Comprobante de recepción</Button></div></div></Card>}
    <Card className="border-0 p-5 shadow-lg"><div className="flex items-center justify-between gap-3"><div><h3 className="text-xl font-bold text-slate-900">Transferencias recientes</h3><p className="text-sm text-slate-600">Solo visibles para el espacio administrativo actual.</p></div><Button type="button" variant="outline" onClick={() => void transfers.refetch()}><RefreshCw className="mr-2 h-4 w-4" />Actualizar</Button></div>{transfers.isLoading ? <p className="mt-4 text-sm text-slate-500">Cargando transferencias…</p> : transfers.data?.length ? <div className="mt-4 space-y-2">{transfers.data.map((item: any) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 p-3"><div><strong className="text-[#0B2B5E]">{item.transferNumber}</strong><p className="text-sm text-slate-600">{item.senderName} → {item.recipientName} · {money(item.amountSent)} {item.currency || "EUR"} · {item.status}</p></div><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" onClick={() => printTransfer(item)}><Printer className="mr-2 h-4 w-4" />Recibo</Button><Button type="button" variant="outline" onClick={() => setDeliveryReceiptTransfer(item)} className="border-emerald-600 text-emerald-700"><Receipt className="mr-2 h-4 w-4" />Recepción</Button></div></div>)}</div> : <p className="mt-4 rounded-lg border border-dashed p-4 text-sm text-slate-500">Aún no hay transferencias registradas.</p>}</Card>
    {deliveryReceiptTransfer && <DeliveryReceiptDialog open={Boolean(deliveryReceiptTransfer)} operationId={Number(deliveryReceiptTransfer.id)} operation="transferencia" reference={String(deliveryReceiptTransfer.transferNumber)} order={deliveryReceiptTransfer.transferNumber} code={null} recipientName={deliveryReceiptTransfer.recipientName} recipientLastName={null} recipientDni={deliveryReceiptTransfer.recipientDocument} brand={branding.isKasega ? "kasega" : "servicom"} onClose={() => setDeliveryReceiptTransfer(null)} />}
  </div>;
}
