...
import { getContractedDocumentServices } from "./userReceipt";
...
export function buildAdminDeliveryTicketHtml(data: {
  ...
  const paymentUi = getPaymentStatusUi(data.paymentStatus);
  const paymentTone = paymentUi.isPaid ? "#047857" : paymentUi.isPending ? "#be123c" : "#334155";
  const paymentBackground = paymentUi.isPaid ? "#ecfdf5" : paymentUi.isPending ? "#fff1f2" : "#f8fafc";
  const paymentHtml = `<div class="row"><div class="label" style="width:130px">ESTADO DE PAGO:</div><div class="value"><strong style="display:inline-block;padding:3px 9px;border-radius:5px;background:${paymentBackground};color:${paymentTone};font-size:12px">${paymentUi.label}</strong></div></div>`;
  ...
  const servicesHtml = contractedServices.length ? `<div class="row"><div class="label" style="width:130px">SERVICIOS CONTRATADOS:</div><div class="value"><strong>${contractedServices.join(", ")}</strong></div></div>` : "";
