...
export function buildReceiptTicketHtml(data: {
  ...
  return `<div class="ticket"><div class="ticket-title">${route.deliveryTitle}</div><div class="ticket-grid"><div><strong>ORDEN:</strong> ${data.order}<br><strong>CÓDIGO:</strong> ${data.code}<br><strong>RUTA:</strong> ${route.route}<br><strong>ORIGEN:</strong> ${route.originPrintLabel}<br><strong>DESTINO:</strong> ${route.destinationPrintLabel}<br><strong>SEDE DE ENTREGA:</strong> ${route.destination.officeLabel}<br><strong>DIRECCIÓN:</strong> ${route.destination.address}<br><strong>RECEPTOR:</strong> ${data.recipient}<br><strong>CELULAR DESTINATARIA:</strong> ${data.recipientPhone}</div><div class="ticket-code">${data.code}</div></div></div>`;
}
...
export async function printUserShipmentReceipt(shipment: any): Promise<void> {
  ...
  try {
    ...
    const isPaid = paymentPresentation.isPaid;
    const paidOptionColor = isPaid ? "#059669" : "#000000";
    const paidOptionBackground = isPaid ? "#ecfdf5" : "transparent";
    const pendingOptionColor = paymentPresentation.isPending ? "#e11d48" : "#000000";
    const pendingOptionBackground = paymentPresentation.isPending ? "#fff1f2" : "transparent";
