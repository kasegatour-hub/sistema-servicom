...
export function getPaymentStatusPresentation(status?: string | null, paymentCondition?: string | null) {
  const isPaid = status === "Pagado";
  const paidInLima = isPaid && typeof paymentCondition === "string" && /\blima\b/i.test(paymentCondition);
  return {
    label: isPaid ? "Pagado" : "No cancelado",
    color: !isPaid ? "#e11d48" : paidInLima ? "#059669" : "#000000",
    background: !isPaid ? "#fff1f2" : paidInLima ? "#ecfdf5" : "transparent",
    isPaid,
    paidInLima,
  };
}
...
export function buildReceiptTicketHtml(data: {
  ...
  return `<div class="ticket"><div class="ticket-title">CONTROL DE ENTREGA — TORINO, ITALIA</div><div class="ticket-grid"><div><strong>ORDEN:</strong> ${data.order}<br><strong>CÓDIGO:</strong> ${data.code}<br><strong>DESTINO:</strong> TORINO, ITALIA<br><strong>RECEPTOR:</strong> ${data.recipient}<br><strong>CELULAR DESTINATARIA:</strong> ${data.recipientPhone}</div><div class="ticket-code">${data.code}</div></div></div>`;
}
...
export async function printUserShipmentReceipt(shipment: any): Promise<void> {
  ...
  try {
    ...
    const paymentPresentation = getPaymentStatusPresentation(shipment.paymentStatus, shipment.paymentCondition);
    const paymentStatus = paymentPresentation.label;
    const isPaid = paymentPresentation.isPaid;
    const paymentColor = paymentPresentation.color;
    const paymentBg = paymentPresentation.background;
    const paidOptionColor = isPaid ? paymentColor : "#000000";
    const paidOptionBackground = isPaid ? paymentBg : "transparent";
    const pendingOptionColor = isPaid ? "#000000" : paymentColor;
    const pendingOptionBackground = isPaid ? "transparent" : paymentBg;
