...
import { getDeclarationLegalText, getRoutePresentation } from "./routeDetails";
...
export function buildReceiptMarkdown(shipment: any): string {
  ...
  const route = getRoutePresentation(shipment.route, delivery.address, shipment.originAddress, shipment.originPoint, shipment.destinationPoint);
  ...
  const senderDocumentLabel = getIdentityDocumentLabel(shipment.senderDocumentType);
...
- **Nombre:** ${sender}
- **${senderDocumentLabel}:** ${shipment.senderDni || "No especificado"}
- **Celular:** ${formatPhoneNumber(shipment.senderPhone) || "No especificado"}

## Destinatario

- **Nombre:** ${recipient}
- **${recipientDocumentLabel}:** ${shipment.recipientDni || "No especificado"}
...
export function buildReceiptRouteSummaryHtml(routeValue?: string | null, destinationAddress?: string | null, destinationPhone?: string | null, originAddress?: string | null, originPhone?: string | null, originPoint?: string | null, destinationPoint?: string | null) {
...
export async function printUserShipmentReceipt(shipment: any): Promise<void> {
  ...
  try {
    ...
    const senderDocumentLabel = escapeHtml(getIdentityDocumentLabel(shipment.senderDocumentType));
    ...
    const recipientDocumentLabel = escapeHtml(getIdentityDocumentLabel(shipment.recipientDocumentType));
    ...
    const delivery = getReceiptDeliveryDetails(shipment, branding);
    const routePresentationBase = getRoutePresentation(shipment.route, delivery.address, shipment.originAddress, shipment.originPoint, shipment.destinationPoint);
