...
import { buildElectronicSignatureHtml, buildReceiptDownloadFilename, getContractedDocumentServices, getReceiptBranding } from "./userReceipt";
...
export async function buildAdminReceiptDocument(input: AdminReceiptDocumentInput) {
  ...
  const route = getRoutePresentation(shipment.route, branding.isKasega ? branding.destinationAddress : shipment.destinationAddress, shipment.originAddress, shipment.originPoint, shipment.destinationPoint);
...
export async function downloadAdminReceiptPdf(input: AdminReceiptDocumentInput, options?: { printWindow?: Window | null }): Promise<string> {
  ...
  const shipmentLabel = shipment.shipmentType === "encomienda" ? "ENCOMIENDA" : "DOCUMENTO";
  const contractedServices = getContractedDocumentServices(shipment);
  const serviceRows: Array<[string, string]> = contractedServices.length ? [["Servicios contratados", contractedServices.join(", ")]] : [];
  ...
  y = addRows([["Ruta", route.route], ["Orden", order], ["Código", code], ["Estado", String(shipment.status || "No especificado")], ["Estado de pago", paymentLabel], ...serviceRows], y);
