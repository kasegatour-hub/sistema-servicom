...
const mocks = vi.hoisted(() => ({
  ...
  requestRecipientChange: { isPending: false, mutateAsync: vi.fn().mockResolvedValue({ requestId: 44, signatureUrl: "https://servicom.test/cambio-destinatario?solicitud=44&token=token-seguro", expiresAt: new Date("2026-08-31T12:00:00.000Z"), accountNotified: true, emailSent: true, requiresManualDelivery: false }) },
...
vi.mock("@/lib/trpc", () => ({
  trpc: {
    ...
    admin: {
      ...
      requestRecipientChange: { useMutation: () => mocks.requestRecipientChange },
...
describe("AdminDashboard Nueva Encomienda", () => {
  ...
  it("muestra ocultamiento reversible al Master Admin de su propio espacio", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Iniciar Sesión" }));
    await waitFor(() => expect(screen.getByRole("button", { name: "Historial" })).toBeTruthy());
    const hideButton = screen.getByRole("button", { name: "Ocultar a Registradores" });
