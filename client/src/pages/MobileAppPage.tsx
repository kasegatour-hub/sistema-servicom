...
export default function MobileAppPage() {
  ...
  return <main className="min-h-[100dvh] max-w-full overflow-x-hidden bg-[#f4f7fb] pb-[calc(6.4rem+env(safe-area-inset-bottom))] text-slate-900"><header className="sticky top-0 z-30 border-b border-slate-200/70 bg-white/95 px-4 pb-3 pt-[max(0.85rem,env(safe-area-inset-top))] backdrop-blur"><div className="mx-auto flex max-w-md items-center justify-between"><div className="flex items-center gap-2"><img src="/manus-storage/servicom_logo_final_e7ce35aa.png" alt="Servicom Internacional" className="h-14 w-14 rounded-2xl bg-slate-50 p-1.5 shadow-sm" /><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#F28C00]">Servicom móvil</p><p className="truncate text-sm font-extrabold text-[#0B2B5E]">Hola, {name} {String(me.lastName || "").trim()}</p><p className="truncate text-[11px] text-slate-500">{me.email}</p></div></div><Button type="button" aria-label="Escanear QR de rastreo" onClick={openScanner} className={`h-11 w-11 rounded-2xl bg-[#0B2B5E] p-0 text-white hover:bg-[#123d78] ${pressClass}`}><ScanLine className="h-5 w-5" /></Button></div></header>
    <div className="mx-auto max-w-md px-4 pt-5">
      ...
      {activeTab === "cuenta" && <section className="space-y-4" aria-label="Cuenta móvil"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#F28C00]">Mi espacio</p><h1 className="mt-1 text-3xl font-extrabold text-[#0B2B5E]">Tu cuenta</h1><p className="mt-1 text-sm text-slate-600">Gestiona tu perfil, recibos y registros sin perderte.</p></div><Card className="rounded-[1.75rem] border-0 bg-white p-5 shadow-sm"><div className="flex items-center gap-3"><span className="rounded-2xl bg-blue-50 p-3 text-[#0B2B5E]"><UserRound className="h-6 w-6" /></span><div className="min-w-0"><p className="truncate font-bold text-[#0B2B5E]">{[me.name, me.lastName].filter(Boolean).join(" ") || "Mi cuenta"}</p><p className="truncate text-sm text-slate-500">{me.email || "Cuenta Servicom"}</p></div></div><Link href={mobileSignInPath} className={`mt-5 flex ${pressClass}`}><Button className="min-h-14 w-full rounded-2xl bg-[#0B2B5E] text-base font-bold text-white hover:bg-[#123d78]">Abrir mi cuenta</Button></Link></Card><Card className="rounded-[1.5rem] border border-blue-100 bg-blue-50 p-4"><p className="text-sm leading-5 text-slate-700"><strong className="text-[#0B2B5E]">Funciones disponibles:</strong> registrar envíos, rastrear y cambiar contraseña.</p></Card><Button type="button" variant="outline" onClick={() => void install()} className={`min-h-13 w-full rounded-2xl border-slate-300 bg-white font-bold text-[#0B2B5E] ${pressClass}`}><Download className="mr-2 h-5 w-5" />{isStandalone ? "App instalada" : "Instalar en este celular"}</Button>{showInstallHelp && !isStandalone && <Card className="rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm text-slate-700"><p><strong>Android:</strong> menú del navegador → «Instalar aplicación».</p><p className="mt-2"><strong>iPhone:</strong> Compartir → «Añadir a pantalla de inicio».</p></Card>}</section>}
