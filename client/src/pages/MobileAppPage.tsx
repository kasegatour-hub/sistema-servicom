...
export default function MobileAppPage() {
  ...
  return <main className="min-h-[100dvh] max-w-full overflow-x-hidden bg-[#f4f7fb] pb-[calc(6.4rem+env(safe-area-inset-bottom))] text-slate-900"><header className="sticky top-0 z-30 border-b border-slate-200/70 bg-white/95 px-4 pb-3 pt-[max(0.85rem,env(safe-area-inset-top))] backdrop-blur"><div className="mx-auto flex max-w-md items-center justify-between"><div className="flex items-center gap-2"><img src="/manus-storage/servicom_logo_final_e7ce35aa.png" alt="Servicom Internacional" className="h-14 w-14 rounded-2xl bg-slate-50 p-1.5 shadow-sm" /><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#F28C00]">Servicom móvil</p><p className="truncate text-sm font-extrabold text-[#0B2B5E]">Hola, {name} {String(me.lastName || "").trim()}</p><p className="truncate text-[11px] text-slate-500">{me.email}</p></div></div><div className="flex items-center gap-2">{canAccessAdmin && <Link href="/admin?from=movil" aria-label="Acceso administrativo" className={`flex ${pressClass}`}><Button type="button" variant="outline" aria-label="Acceso Admin" className="h-11 rounded-2xl border-[#F28C00] px-3 text-xs font-bold text-[#A65A00] hover:bg-orange-50">Admin</Button></Link>}<Button type="button" aria-label="Escanear QR de rastreo" onClick={openScanner} className={`h-11 w-11 rounded-2xl bg-[#0B2B5E] p-0 text-white hover:bg-[#123d78] ${pressClass}`}><ScanLine className="h-5 w-5" /></Button></div></div></header>
