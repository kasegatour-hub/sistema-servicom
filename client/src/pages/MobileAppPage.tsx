...
export default function MobileAppPage() {
  ...
  return <main className="min-h-[100dvh] max-w-full overflow-x-hidden bg-[#f4f7fb] pb-[calc(6.4rem+env(safe-area-inset-bottom))] text-slate-900"><header className="sticky top-0 z-30 border-b border-slate-200/70 bg-white/95 px-4 pb-3 pt-[max(0.85rem,env(safe-area-inset-top))] backdrop-blur"><div className="mx-auto flex max-w-md items-center justify-between"><div className="flex items-center gap-2"><img src="/manus-storage/servicom_logo_final_e7ce35aa.png" alt="Servicom Internacional" className="h-14 w-14 rounded-2xl bg-slate-50 p-1.5 shadow-sm" /><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#F28C00]">Servicom móvil</p><p className="truncate text-sm font-extrabold text-[#0B2B5E]">Hola, {name} {String(me.lastName || "").trim()}</p><p className="truncate text-[11px] text-slate-500">{me.email}</p></div></div><div className="flex items-center gap-2"><Link href="/" aria-label="Volver a Inicio web" className={`flex ${pressClass}`}><Button type="button" variant="outline" className="h-11 rounded-2xl border-[#0B2B5E] px-3 text-xs font-bold text-[#0B2B5E] hover:bg-blue-50"><Home className="mr-1.5 h-4 w-4" />Inicio</Button></Link>{canAccessAdmin && <Link href="/admin?from=movil" aria-label="Acceso administrativo" className={`flex ${pressClass}`}><Button type="button" variant="outline" aria-label="Acceso Admin" className="h-11 rounded-2xl border-[#F28C00] px-3 text-xs font-bold text-[#A65A00] hover:bg-orange-50">Admin</Button></Link>}</div></div></header>
    <div className="mx-auto max-w-md px-4 pt-5">
      ...
      {activeTab === "cuenta" && <section className="space-y-4" aria-label="Cuenta móvil"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#F28C00]">Mi cuenta</p><h1 className="mt-1 text-3xl font-extrabold text-[#0B2B5E]">Tus datos personales</h1><p className="mt-1 text-sm leading-5 text-slate-600">Administra únicamente tu perfil, tus fotos personales, tu biografía y tu contraseña.</p></div><Card className="rounded-[1.75rem] border-0 bg-white p-5 shadow-sm"><div className="flex items-center gap-3"><span className="rounded-2xl bg-blue-50 p-3 text-[#0B2B5E]"><UserRound className="h-6 w-6" /></span><div className="min-w-0"><p className="truncate font-bold text-[#0B2B5E]">{[me.name, me.lastName].filter(Boolean).join(" ") || "Mi cuenta"}</p><p className="truncate text-sm text-slate-500">{me.email || "Cuenta Servicom"}</p></div></div><div className="mt-5 grid gap-3"><Link href="/cuenta?mobile=1&workspace=perfil" className={`flex ${pressClass}`}><Button className={`min-h-16 w-full rounded-2xl bg-[#0B2B5E] text-base font-extrabold text-white hover:bg-[#123d78] ${pressClass}`}><UserRound className="mr-3 h-6 w-6" />Perfil, fotos y biografía</Button></Link><Link href="/cuenta?mobile=1&workspace=seguridad" className={`flex ${pressClass}`}><Button variant="outline" className={`min-h-16 w-full rounded-2xl border-2 border-[#F28C00] text-base font-extrabold text-[#A65A00] hover:bg-orange-50 ${pressClass}`}><ShieldCheck className="mr-3 h-6 w-6" />Cambiar contraseña</Button></Link></div></Card><Card className="rounded-[1.5rem] border border-blue-100 bg-blue-50 p-4"><p className="text-sm leading-5 text-slate-700"><strong className="text-[#0B2B5E]">Solo gestión personal.</strong> Para rastrear o registrar un envío, vuelve a Inicio y usa sus botones grandes.</p></Card></section>}
