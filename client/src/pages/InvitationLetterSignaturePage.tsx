...
export default function InvitationLetterSignaturePage() {
  ...
  if (letterQuery.isLoading) {
    return <main className="min-h-screen bg-slate-50 p-6"><Card className="mx-auto flex max-w-xl items-center justify-center gap-3 p-8 text-[#0B2B5E]"><Loader2 className="h-5 w-5 animate-spin" />Preparando la Carta…</Card></main>;
  }
  ...
  return <main className="min-h-screen bg-slate-50 p-4 sm:p-8">
    <Card className="mx-auto max-w-2xl overflow-hidden border-0 shadow-xl">
      <header className="bg-[#0B2B5E] px-6 py-5 text-white">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><p className="text-sm font-semibold text-orange-300">SERVICOM INTERNACIONAL</p><h1 className="mt-1 text-2xl font-bold">Firma de Carta de invitación</h1><p className="mt-1 text-sm text-blue-100">Revisa el estado y firma con tu dedo o mouse.</p></div>
          <Button type="button" size="sm" variant="outline" onClick={closeSignaturePage} className="border-white/60 bg-white/10 text-white hover:bg-white hover:text-[#0B2B5E]"><X className="mr-1 h-4 w-4" />Cerrar ventana</Button>
        </div>
      </header>
      <div className="space-y-5 p-6">
        <section className={`rounded-xl border p-4 ${signed ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}><div className="flex gap-3"><div className={`mt-0.5 rounded-full p-2 ${signed ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>{signed ? <CheckCircle2 className="h-5 w-5" /> : <FileSignature className="h-5 w-5" />}</div><div><h2 className="font-bold text-slate-900">{signed ? "Después: Carta firmada" : "Antes: Carta pendiente de firma"}</h2><p className="mt-1 text-sm text-slate-700">Invitante: <strong>{letter.inviterName}</strong><br />Invitado: <strong>{letter.inviteeName}</strong></p>{signed && <p className="mt-2 text-sm text-emerald-800">Firma registrada el {letter.signedAt ? new Date(letter.signedAt).toLocaleString("es-PE") : "momento indicado"}.</p>}</div></div></section>
