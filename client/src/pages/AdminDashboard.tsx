...
function ProvinceSenderPicker({
  senders,
  value,
  onSelect,
}: {
  senders: ProvinceSenderRecord[];
  value: string;
  onSelect: (sender: ProvinceSenderRecord | null) => void;
}) {
  const [query, setQuery] = useState("");
  const activeSenders = useMemo(() => senders.filter(sender => Boolean(sender.isActive)), [senders]);
  const visibleSenders = useMemo(() => activeSenders
    .map(sender => ({ sender, score: getFuzzySearchScore(query, `${sender.name} ${sender.lastName} ${sender.dni || ""} ${sender.phone || ""}`) }))
    .filter(result => result.score > 0)
    .sort((left, right) => right.score - left.score)
    .map(result => result.sender), [activeSenders, query]);
  return <div className="mt-3 space-y-2">
    <label className="block text-sm font-semibold text-[#0B2B5E]" htmlFor="province-sender-search">Buscar remitente provincial</label>
    <Input id="province-sender-search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Nombre, apellido, DNI o celular" className="h-11 bg-white" />
    <select aria-label="Remitente provincial guardado" className="h-11 w-full rounded-md border border-slate-300 bg-white px-3 text-sm text-[#0B2B5E]" value={value} onChange={(event) => {
      const sender = activeSenders.find(item => `${item.name}|${item.lastName}|${item.dni || ""}` === event.target.value) || null;
      onSelect(sender);
    }}>
      <option value="">Selecciona un remitente activo o escribe uno nuevo</option>
      {visibleSenders.map(sender => <option key={sender.id} value={`${sender.name}|${sender.lastName}|${sender.dni || ""}`}>{sender.name} {sender.lastName} · DNI {sender.dni || "No indicado"} · {sender.phone || "Sin celular"}</option>)}
    </select>
    {query.trim() && !visibleSenders.length && <p className="text-xs font-medium text-slate-600">No se encontró un remitente activo. Puedes escribir uno nuevo en los campos inferiores.</p>}
  </div>;
}
...
const createShipmentSchema = z.object({
  ...
  limaTorinoTransferMode: z.enum(["dhl_recogida", "persona_autorizada"]).optional(),
...
const updateStatusSchema = z.object({
  ...
  manualPriceEur: z.union([z.string(), z.number()]).optional().nullable(),
  manualPriceCurrency: z.enum(["EUR", "USD", "PEN"]).optional(),
...
export default function AdminDashboard() {
  ...
  const auditTotalPages = Math.max(1, Math.ceil(shipmentAudit.length / auditPageSize));
  const visibleShipmentAudit = shipmentAudit.slice((auditPage - 1) * auditPageSize, auditPage * auditPageSize);
  ...
  const createForm = useForm<any>({
    ...
    defaultValues: {
      ...
      provinceSenderDni: '',
      provinceSenderPhone: '',
      limaTorinoTransferMode: undefined,
      deliveryPersonName: '',
      deliveryPersonLastName: '',
      deliveryPersonDni: '',
      deliveryPersonPhone: '',
      deliveryLocationType: 'direccion',
  ...
  const createPricingPreview = useMemo(() => calculateAdminShipmentPricing({
    ...
    manualPriceCurrency: createForm.watch("manualPriceCurrency"),
  ...
  const createFormTone = isParcelCreateForm
    ? { shell: "border-orange-200 bg-orange-50/60", mobileBorder: "border-orange-300", heading: "border-orange-200 bg-orange-100", text: "text-[#9A5700]", activeStep: "bg-[#F28C00]", progress: "bg-[#F28C00]" }
    : { shell: "border-blue-200 bg-blue-50/60", mobileBorder: "border-[#0B2B5E]", heading: "border-blue-200 bg-blue-50", text: "text-[#0B2B5E]", activeStep: "bg-[#0B2B5E]", progress: "bg-[#0B2B5E]" };
  ...
  useEffect(() => {
    if (selectedShipmentType !== "documento" && createForm.getValues("requiresApostilleService")) {
  ...
  useEffect(() => {
    if (updateShipmentType !== "documento" && (updateForm.getValues("requiresApostilleService") || updateForm.getValues("requiresTranslationService"))) {
  ...
  const updatePricingCurrency = derivePricingCurrency(normalizeIndependentEndpoints({ route: updateShipmentRoute, isProvinceDelivery: updateProvinceEnabled }));
  ...
  const updateProvinceCustomerPrice = updateProvinceEnabled ? Number(updateForm.watch("provinceCustomerPriceEur") || 0) : 0;
  const updateProvinceExtraPrice = updateProvinceEnabled ? Number(updateForm.watch("provinceExtraPriceEur") || 0) : 0;
  ...
  const updatePricingPreview = useMemo(() => calculateAdminShipmentPricing({
    ...
    manualPriceCurrency: updateForm.watch("manualPriceCurrency"),
  ...
  const openShipmentUpdate = (shipment: any) => {
    ...
    updateForm.reset({
      ...
      manualPriceCurrency: shipment.manualPriceCurrency || "EUR",
  ...
  const shipmentDetailsText = (shipment: any) => {
    ...
    return [`ENVÍO ${shipment.shipmentType === "encomienda" ? "ENCOMIENDA" : "DOCUMENTO"}`, `Orden: ${shipment.orderNumber || "No indicado"}`, `Código: ${shipment.code || "No indicado"}`, `Estado: ${shipment.status || "No indicado"}`, `Estado de pago: ${shipment.paymentStatus || "No indicado"}`, `Ruta: ${shipment.route || "No indicado"}`, `Remitente: ${[shipment.senderName, shipment.senderLastName].filter(Boolean).join(" ") || "No indicado"}`, `Teléfono remitente: ${shipment.senderPhone || "No indicado"}`, `Documento remitente: ${shipment.senderDni || "No indicado"}`, `Destinatario: ${[shipment.recipientName, shipment.recipientLastName].filter(Boolean).join(" ") || "No indicado"}`, `Teléfono destinatario: ${shipment.recipientPhone || "No indicado"}`, `Documento destinatario: ${shipment.recipientDni || "No indicado"}`, `Origen: ${shipment.originAddress || "No indicado"}`, `Destino / sede: ${shipment.destinationAddress || "No indicado"}`, `Modalidad Lima–Torino: ${shipment.limaTorinoTransferMode === "dhl_recogida" ? "DHL recoge el documento" : shipment.limaTorinoTransferMode === "persona_autorizada" ? "Entrega a persona autorizada" : "No aplica"}`, `Persona autorizada: ${[shipment.deliveryPersonName, shipment.deliveryPersonLastName].filter(Boolean).join(" ") || "No aplica"}`, `Lugar de entrega: ${shipment.deliveryLocationType === "aeropuerto_jorge_chavez" ? "Nuevo Aeropuerto Internacional Jorge Chávez" : shipment.deliveryLocationAddress || "No aplica"}`, `Operador provincial: ${shipment.provinceCarrier || "No aplica"}`, `Peso: ${shipment.weightKg ? `${shipment.weightKg} kg` : "No aplica"}`, `Precio final: ${formatShipmentAmount(shipment.finalPriceEur ?? shipment.basePriceEur ?? shipment.manualPriceEur, shipment)}`, `Notas: ${shipment.notes || "Sin notas"}`, `Checklist: ${checklist.length ? checklist.join(", ") : "Sin checklist"}`].join("\n");
  };
  ...
  const shipmentActionButtons = (shipment: any) => <>
    ...
    {admin?.role === "superadmin" && <Button onClick={() => { setAuditPage(1); setAuditShipmentId(shipment.id); }} size="sm" variant="outline" className="border-slate-400 text-slate-700 hover:bg-slate-100">Historial</Button>}
  </>;
  ...
  return (
    <div className="admin-surface min-h-screen bg-gradient-to-b from-blue-50 to-white">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        {adminWorkspace === "crear" && createRecordTab === "transferencia" && <TransferWorkspace admin={admin} mobileRegistrationMode={mobileRegistrationMode} />}
        ...
        <Card className={`mb-8 border-0 p-6 shadow-lg ${adminWorkspace === "crear" ? "" : "hidden"}`}>
          ...
          {createRecordTab !== "transferencia" && showCreateForm && (<>
            <form noValidate={isYeslyExceptionForm} onSubmit={createForm.handleSubmit(handleCreateShipment, handleCreateShipmentInvalid)} className={`space-y-4 rounded-2xl border-2 p-4 shadow-sm ${createFormTone.shell}`}>
              ...
              <div className="flex flex-col rounded-xl border border-white/80 bg-white/75 p-4">
                {mobileRegistrationMode && <div className={`mb-4 rounded-xl border-2 bg-white p-4 ${createFormTone.mobileBorder}`} role="region" aria-label="Pasos del registro móvil"><div className="flex items-center justify-between gap-2"><p className={`text-sm font-extrabold ${createFormTone.text}`}>Registro móvil</p><span className="text-xs font-bold text-slate-500">Paso {mobileRegistrationStep} de 3</span></div><div className="mt-3 grid grid-cols-3 gap-2">{([1, 2, 3] as const).map(step => <button key={step} type="button" onClick={() => setMobileRegistrationStep(step)} className={`min-h-11 rounded-lg px-2 text-xs font-bold transition ${mobileRegistrationStep === step ? `${createFormTone.activeStep} text-white` : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`} aria-current={mobileRegistrationStep === step ? "step" : undefined}>{step === 1 ? "Sede y pago" : step === 2 ? "Personas" : "Contenido"}</button>)}</div><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full transition-all ${createFormTone.progress}`} style={{ width: `${mobileRegistrationStep * 33.333}%` }} /></div><p className="mt-2 text-xs leading-5 text-slate-600">{mobileRegistrationStep === 1 ? "Define ruta, tipo, entrega y estado de pago." : mobileRegistrationStep === 2 ? "Completa remitente y destinatario con sus datos." : "Revisa el contenido, adjunta una foto opcional y crea el registro."}</p></div>}
                <div className={`order-0 mb-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border px-4 py-3 ${createFormTone.heading}`}>
                  <div>
                    <p className={`text-xs font-semibold uppercase tracking-wide ${createFormTone.text}`}>Formulario de registro</p>
                    <p className={`text-lg font-bold ${createFormTone.text}`}>{selectedShipmentType === "encomienda" ? "Nueva encomienda" : "Nuevo documento"}</p>
                  </div>
                ...
                <div className={`order-40 mt-4 grid grid-cols-1 gap-3 rounded-lg border border-emerald-200 bg-emerald-50 p-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-end ${mobileSectionClass(1)}`}>
                  ...
                  {selectedShipmentType === "documento" && <p className="mb-3 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-semibold text-[#0B2B5E]">Apostilla y traducción disponibles para cualquier ruta documental.</p>}
                  <div className="mt-4 grid grid-cols-1 gap-4 border-t border-slate-200 pt-4 md:grid-cols-3">
                    ...
                    />
                    <DocumentPricePreview docType={selectedDocType} sheetCount={Number(createForm.watch("sheetCount")) || 1} additionalTotalEur={additionalDocumentAutoTotal} penPerEur={selectedPenPerEur} manualPriceEur={createForm.watch("manualPriceEur")} manualPriceCurrency={createForm.watch("manualPriceCurrency")} pricingCurrency={createPricingPreview.pricingCurrency} extraPriceEur={createForm.watch("extraPriceEur")} extraDiscountEur={createForm.watch("extraDiscountEur")} />
                  </div>
                                     {selectedShipmentType === "documento" && (
                     <>
...
<div><label className="mb-1 block text-sm font-semibold text-slate-700">Extra provincial proporcional ({selectedPricingCurrency})</label><Input type="number" min="0" step="0.01" placeholder={watchedWeightKg > 10 ? `Automático: ${automaticProvinceExtraPrice.toFixed(2)}` : "0.00"} {...createForm.register("provinceExtraPriceEur")} /><p className="mt-1 text-xs text-slate-600">Solo para provincia: se calcula automáticamente como los kg por encima de 10 × 2,00 EUR; puedes modificarlo si lo deseas. Este importe no impide crear la encomienda.</p></div></div><div><label className="mb-1 block text-sm font-semibold text-slate-700">Costo operativo (soles)</label><Input type="number" min="0" step="0.01" placeholder={selectedShipmentType === "documento" ? "8.00 automático" : "Ej. 12.00"} {...createForm.register("provinceOperationalCostSoles")} /><p className="mt-1 text-xs text-slate-600">Documentos: S/ 8.00 por defecto.</p></div><div className="rounded-md bg-white px-3 py-2 text-sm text-slate-700"><strong>Courier:</strong> Se usará la agencia seleccionada arriba.</div></div></></section>}
        ...
        <Card className={`mt-0 border-0 p-6 shadow-md ${adminWorkspace === "papelera" ? "" : "hidden"}`}>
          ...
          {filteredDeletedShipments.length === 0 ? <p className="mt-4 text-sm text-slate-500">No hay envíos eliminados que coincidan con los filtros seleccionados.</p> : <div className="mt-4 space-y-2">
            {deletedPagination.items.map((shipment: any) => <div key={shipment.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
              ...
              <div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" disabled={restoreMutation.isPending} onClick={() => restoreMutation.mutate({ shipmentId: shipment.id })}><RotateCcw className="mr-2 h-3.5 w-3.5" /> Restaurar</Button>{admin?.role === "superadmin" && <Button size="sm" variant="outline" onClick={() => { setAuditPage(1); setAuditShipmentId(shipment.id); }}>Historial</Button>}</div>
            </div>)}
        ...
        {admin?.role === "superadmin" && auditShipmentId && <div className="fixed inset-0 z-60 flex items-center justify-center bg-slate-950/60 p-4" role="dialog" aria-modal="true" aria-labelledby="shipment-history-title"><Card className="max-h-[90dvh] w-full max-w-3xl overflow-y-auto border-0 p-5 shadow-2xl sm:p-7" aria-label="Historial administrativo del envío">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 id="shipment-history-title" className="text-lg font-bold text-[#0B2B5E]">Historial de cambios del envío</h2><p className="mt-1 text-xs text-slate-500">Visible únicamente para el Master Admin. Incluye quién eliminó, actualizó, restauró o completó una firma.</p></div><Button type="button" size="sm" variant="outline" aria-label="Cerrar historial" onClick={() => { setAuditShipmentId(null); setAuditPage(1); }}>×</Button></div>
          {isLoadingShipmentAudit ? <p className="mt-4 text-sm text-slate-500">Cargando historial…</p> : shipmentAudit.length === 0 ? <p className="mt-4 text-sm text-slate-500">No hay registros de auditoría disponibles para este envío.</p> : <ol className="mt-4 space-y-3">{visibleShipmentAudit.map((entry: any) => <li key={entry.id} className="rounded-lg border border-slate-200 bg-slate-50 p-3"><p className="text-sm font-semibold text-[#0B2B5E]">{({ created: "Registro creado", updated: "Datos o estado actualizado", deleted: "Envío eliminado", restored: "Envío restaurado", price_updated: "Precio actualizado", signature_requested: "Firma solicitada", signature_completed: "Firma completada", feedback_added: "Retroalimentación añadida", hidden_from_registradores: "Envío oculto para Registradores", shown_to_registradores: "Envío mostrado a Registradores" } as Record<string, string>)[entry.action] || entry.action}</p><p className="mt-1 text-xs text-slate-600">Realizado por: <strong>{entry.actorDisplayName || "No indicado"}</strong> · {entry.createdAt ? new Date(entry.createdAt).toLocaleString() : "sin fecha"}</p>{entry.reason && <p className="mt-1 text-xs text-slate-600">Motivo: {entry.reason}</p>}</li>)}</ol>}{!isLoadingShipmentAudit && shipmentAudit.length > 0 && <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4"><p className="text-sm text-slate-600">Mostrando {(auditPage - 1) * auditPageSize + 1}–{Math.min(auditPage * auditPageSize, shipmentAudit.length)} de {shipmentAudit.length}</p><div className="flex items-center gap-2"><Button type="button" size="sm" variant="outline" disabled={auditPage === 1} onClick={() => setAuditPage(page => Math.max(1, page - 1))}>Anterior</Button><span className="min-w-24 text-center text-sm font-medium text-slate-700">Página {auditPage} de {auditTotalPages}</span><Button type="button" size="sm" variant="outline" disabled={auditPage >= auditTotalPages} onClick={() => setAuditPage(page => Math.min(auditTotalPages, page + 1))}>Siguiente</Button></div></div>}
        </Card></div>}
	...
	<div>
                  ...
                  {updateForm.watch("shipmentType") === "encomienda" ? <>
                    ...
                    <div>
                      ...
                      <select {...updateForm.register("pricingMode")} className="w-full p-2 bg-white border-2 border-slate-200 rounded-md text-sm font-medium focus:border-primary">
                        ...
                        <option value="manual">Precio manual en EUR, USD o PEN</option>
                      </select>
                    </div>
                  </> : <>
                    <div>
                    ...
                    <div className="md:col-span-2 rounded-xl border border-orange-200 bg-orange-50 p-4">
                      ...
                      {(
                        ...
                        <div className="mt-3 grid gap-3 sm:grid-cols-2">
                          ...
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Extra provincial proporcional ({updatePricingCurrency})</label>
                            <Input type="number" min="0" step="0.01" {...updateForm.register("provinceExtraPriceEur")} placeholder={Number(updateForm.watch("weightKg") || 0) > 10 ? `Automático: ${((Number(updateForm.watch("weightKg") || 0) - 10) * 2).toFixed(2)}` : "0.00"} />
                            <p className="mt-1 text-xs text-slate-600">Separa este extra del importe extra internacional. Sobre 10 kg se calcula automáticamente a 2,00 EUR por kg.</p>
                          </div>
                          <div className="sm:col-span-2">
                  ...
                  {updateForm.watch("pricingMode") === "manual" && <div className="md:col-span-2"><div className="mb-1 flex items-center justify-between gap-3"><label className="block text-sm font-medium text-gray-700">Precio manual para agencia externa</label><select aria-label="Moneda del precio manual de actualización" {...updateForm.register("manualPriceCurrency")} className="h-9 rounded-md border-2 border-slate-200 bg-white px-2 text-sm font-semibold text-[#0B2B5E]"><option value="EUR">EUR €</option><option value="USD">USD $</option><option value="PEN">PEN S/</option></select></div><Input type="number" min="0" step="0.01" {...updateForm.register("manualPriceEur")} placeholder="Ej.: 25.00" /><p className="mt-1 text-xs text-slate-500">La moneda seleccionada se conserva en contabilidad, notas y documentos.</p></div>}
                  ...
                  {updateForm.watch("shipmentType") === "documento" && <div className="md:col-span-2 rounded-xl border-2 border-blue-200 bg-blue-50 p-3"><p className="text-xs font-bold uppercase tracking-wide text-[#0B2B5E]">Precio actualizado</p><DocumentPricePreview docType={(updateForm.watch("docType") || "apostillado") as "simple" | "apostillado"} sheetCount={Number(updateForm.watch("sheetCount") || 1)} manualPriceEur={updateForm.watch("pricingMode") === "manual" ? updateForm.watch("manualPriceEur") : null} manualPriceCurrency={updateForm.watch("manualPriceCurrency")} pricingCurrency={updatePricingPreview.pricingCurrency} extraPriceEur={updateForm.watch("extraPriceEur")} extraDiscountEur={updateForm.watch("extraDiscountEur")} serviceTotalEur={updatePricingPreview.servicePriceEur} serviceSummary={[updateForm.watch("requiresApostilleService") ? "apostilla" : "", updateForm.watch("requiresTranslationService") ? "traducción" : ""].filter(Boolean).join(" + ")} /></div>}
                  {updateForm.watch("shipmentType") === "encomienda" && <div className="md:col-span-2 rounded-xl border-2 border-blue-200 bg-blue-50 p-4" aria-live="polite"><p className="text-xs font-bold uppercase tracking-wide text-[#0B2B5E]">Precio actualizado</p><p className="mt-1 text-3xl font-extrabold tabular-nums text-[#0B2B5E]">{formatShipmentAmount(updateVisibleParcelTotal, { manualPriceEur: updateForm.watch("pricingMode") === "manual" ? updateForm.watch("manualPriceEur") : null, manualPriceCurrency: updateForm.watch("manualPriceCurrency"), pricingCurrency: updatePricingPreview.pricingCurrency })}</p><p className="mt-1 text-sm text-slate-700">Se recalcula con el peso, la tarifa, el extra provincial y la agencia seleccionada.</p></div>}
                </div>
